...
struct TrainingGuardState_42 {
    ...
    [[nodiscard]] core::ByteVector serialize_payload() const {
        ...
        if (name_chars > 0xFF) {
            throw std::runtime_error("TrainingGuardState_42 guard_nickname is too long");
        }
    ...
    static TrainingGuardState_42 deserialize(std::span<const std::uint8_t> payload) {
        ByteReader reader(payload);
        TrainingGuardState_42 packet;
